...
describe("MasterAdaSwap", function(){
    ...
    describe('3. Creating pools with different time lock', async()=>{
        ...
        it('Pool with time lock - 60 days',async()=>{
            await chef.connect(ADMIN).add(10, lpToken.address, 3, rewarder.address);

            expect((await chef.isExistPool(lpToken.address, 3))).to.be.equal(true);
            expect((await chef.adaswapPerSecond())).to.be.equal(getBigNumber(10));
            ...
            await chef.connect(TOM).deposit(lpToken.address, TOM.address, getBigNumber(1), 3);
            await chef.connect(GOR).deposit(lpToken.address, GOR.address, getBigNumber(2), 3);
            await chef.connect(TOR).deposit(lpToken.address, TOR.address, getBigNumber(3), 3);
            await chef.connect(YOR).deposit(lpToken.address, YOR.address, getBigNumber(4), 3);

            await advanceIncreaseTime(1);

            pendingAdaSwap1_ = await chef.pendingAdaSwap(lpToken.address, TOM.address, 3);
            pendingAdaSwap2_ = await chef.pendingAdaSwap(lpToken.address, GOR.address, 3);
            pendingAdaSwap3_ = await chef.pendingAdaSwap(lpToken.address, TOR.address, 3);
            pendingAdaSwap4_ = await chef.pendingAdaSwap(lpToken.address, YOR.address, 3);
        ...
        it('Pool with time lock - 365 days ', async()=>{
            ...
            await chef.connect(ADMIN).add(5, lpToken.address, 5, rewarder.address);

            expect((await chef.isExistPool(lpToken.address, 5))).to.be.equal(true);
            expect((await chef.adaswapPerSecond())).to.be.equal(getBigNumber(10));
            ...
            await chef.connect(BOB).deposit(lpToken.address, BOB.address, getBigNumber(5), 5);
            await chef.connect(ALICE).deposit(lpToken.address, ALICE.address, getBigNumber(4), 5);
            await chef.connect(STEAVE).deposit(lpToken.address, STEAVE.address, getBigNumber(3), 5);

            await advanceIncreaseTime(1);
            
            _pendingAdaSwap1 = await chef.pendingAdaSwap(lpToken.address, BOB.address, 5);
            _pendingAdaSwap2 = await chef.pendingAdaSwap(lpToken.address, ALICE.address, 5);
            _pendingAdaSwap3 = await chef.pendingAdaSwap(lpToken.address, STEAVE.address, 5);
