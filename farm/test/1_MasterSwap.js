...
describe("MasterAdaSwap", function(){
    ...
    describe('2. Creating pools with different allocation points',()=>{
        it('Create pool with 2-4 users', async()=>{
            ...
            await lpTokenSecond.connect(BOB).approve(chef.address, getBigNumber(10));
            await lpTokenSecond.connect(ALICE).approve(chef.address, getBigNumber(10));
            await lpTokenSecond.connect(STEAVE).approve(chef.address, getBigNumber(10));
            await lpTokenSecond.connect(SAM).approve(chef.address, getBigNumber(10));
