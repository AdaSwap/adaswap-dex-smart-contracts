...
const main = async () => {
  ...
  const { FEE_ADDRESS } = constants[network.name];

  const AdaswapFactory = await deploy("AdaswapFactory", [deployer.address]);

  setTimeout(()=>{}, 5000)

  let initCodeHash = await AdaswapFactory.pairCodeHash()
  console.log('Pair code hash --> ', initCodeHash)

  await AdaswapFactory.setFeeTo(FEE_ADDRESS)
  console.log('feeTo address setted - ', FEE_ADDRESS)
